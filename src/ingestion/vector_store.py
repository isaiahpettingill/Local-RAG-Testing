...
def insert_chunk(text: str, vector: list[float]) -> int:
    with get_knowledgebase_conn() as conn:
        cursor = conn.execute(
            "INSERT INTO chunks (text, vector) VALUES (?, ?)",
            (text, _pack_vector(vector)),
        )
...
def execute_fts(query: str, limit: int = 5) -> list[dict[str, Any]]:
    with get_knowledgebase_conn() as conn:
        ...
        return [{"id": r["rowid"], "text": r["text"]} for r in rows]
...
def execute_vector(query_vector: list[float], limit: int = 5) -> list[dict[str, Any]]:
    ...
    with get_knowledgebase_conn() as conn:
        try:
            rows = conn.execute(
                "SELECT rowid, text FROM chunks_vec WHERE embedding MATCH ? LIMIT ?",
                (packed, limit),
            ).fetchall()
        except Exception:
            rows = conn.execute(
                "SELECT rowid, text FROM chunks WHERE rowid IN (SELECT rowid FROM chunks LIMIT ?)",
                (limit,),
            ).fetchall()
        return [{"id": r["rowid"], "text": r["text"]} for r in rows]
...
def reciprocal_rank_fusion(
    ...
    return [{"id": i, "text": id_to_text[i]} for i in sorted_ids[:limit]]
